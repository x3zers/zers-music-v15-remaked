...
messagef.edit({content: "🎵 | Müzik Bitti. Daha Fazla İstermisin ?", embeds: [], components: []}).catch(err => {})
})
})
})
...
interaction.reply({content: "Speed online!", ephemeral: true})
  ...
  if (interaction.customId === "yavaşmod") {
    const queue = client.distube.getQueue(interaction);
       if (!queue) return interaction.reply(`There is no song on the list yet.`)
    let data = db.fetch(`music_${interaction.guild.id}`)
    if (!data) return interaction.reply({content: "I'm sorry.Error **404**", ephemeral: true})
...
interaction.reply({content: "Bassboost Aktif!", ephemeral: true})
  }
  if (interaction.customId === "soru") {
    const queue = client.distube.getQueue(interaction);
       if (!queue) return interaction.reply(`Kuyrukta şarkı bulunamadı!`)
    let data = db.fetch(`music_${interaction.guild.id}`)
    if (!data) return interaction.reply({content: "Üzgünüm .Error **404**", ephemeral: true})
...
.addComponents(
  ...
  .setURL("https://discord.gg/63SMHa6Gc2")
)
...
client.on('interactionCreate', async interaction => {
  ...
  const queue = client.distube.getQueue(interaction);
     ...
     if (isNaN(volume)) return interaction.reply("Ayarlamak İstediğin Boyutu Yaz")
     ...
     interaction.reply("Müziğin sesini başarıyla ayarladın **"+volume+"**")
}
})
client.login(token)
